...
class NelderMeadApp:
    ...
    def _parse_config(self) -> tuple:
        ...
        x0 = np.array([
            float(v.strip())
            for v in self._x0_entry.get().split(",")
        ])
    ...
    def _start(self) -> None:
        ...
        self._plot_manager.clear()

        # Фоновый поток
        thread = threading.Thread(target=self._worker, daemon=True)
        thread.start()

        # Запускаем цикл обновления графиков
        self._update_loop()
    ...
    def _update_loop(self) -> None:
        ...
        if self._history:
            ...
            simplexes = [s.simplex_points for s in self._history]

            self._plot_manager.draw_convergence(
                iterations,
                best,
                worst,
                simplexes,
                function=self._objective
            )
    ...
    def _on_finish(self, result) -> None:
        ...
        # Финальная перерисовка графиков после завершения
        if self._history:
            iterations = [s.iteration for s in self._history]
            best = [s.best_f for s in self._history]
            worst = [s.worst_f for s in self._history]
